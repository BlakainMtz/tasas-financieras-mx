...
# =========================
# El anti-bot de Santander/Openbank (Akamai) suele bloquear las IPs de GitHub
# Actions y la UA headless, por lo que el scrape en vivo casi siempre falla
# desde el workflow y regresaba None (celda en blanco). Igual que Banco Plata,
# si el scrape en vivo no obtiene datos se usa este valor verificado a mano.
# ACTUALIZA a mano cuando Openbank cambie su oferta (verificado 07-sep-2026,
# vigente al 07-feb-2027, fuente: openbank.mx/cuenta-debito-open-plus):
#   - Open + (Apartados Open): 13% anual fijo sobre los PRIMEROS $30,000 con
#     una compra/pago/recarga al mes. El excedente rinde 7% (hasta $1M) y 6.5%.
...
# =========================
# FUNCIÓN SUPERTASAS / CREDICLUB (scraping SPA Framer)
# En sep-2026 Supertasas migró a Crediclub: las tasas ahora viven en
# https://crediclub.com/inversión  (con acento). El sitio es una SPA de Framer:
#   - El slug acentuado NO tiene archivo estático propio, así que un GET directo
#     o un reload a esa ruta REBOTA al home (el CDN sirve el fallback = home) y
#     NO trae las tasas. Por eso `requests` ya no sirve.
#   - Solo la navegación INTERNA renderiza las tasas: hay que abrir el HOME con
#     Playwright y hacer click en el enlace "Conocer más" (href con 'inversi'),
#     que dispara el router de Framer (pushState) y monta la página de inversión.
# El texto renderizado conserva el mismo formato de Supertasas (tasa y luego
# etiqueta), por lo que _parsear_texto_supertasas() sigue siendo válido:
#     9.20%  Plazo de 364 días
#     8.50%  Plazo de 364 días (Con intereses cada 28 días)  <- se ignora
#     8.30%  Plazo de 182 días
#     6.60%  Plazo de 91 días
#     6.40%  Plazo de 28 días
#     6.30%  A la Vista
# =========================
...
# URL de la página de inversión de Crediclub (con acento). El link interno del
# home apunta a "./inversión"; el home siempre carga y desde ahí se navega.
CREDICLUB_HOME = "https://crediclub.com/"
CREDICLUB_INVERSION = "https://crediclub.com/inversión"
 
 
def obtener_tasas_supertasas(browser=None):
    """Obtiene las tasas de Crediclub (antes Supertasas) desde la SPA de Framer.
 
    Requiere Playwright: `requests` no sirve porque el GET directo a la ruta
    acentuada rebota al home. Se abre el HOME y se hace click en el enlace de
    inversión para que el router de Framer monte la página con las tasas.
    """
    resultado_vacio = {"a_la_vista": None, "1_mes": None, "3_meses": None, "6_meses": None, "1_ano": None}
 
    if not browser:
        print("Crediclub/Supertasas: se requiere Playwright (browser) para la SPA de Framer")
        return resultado_vacio
 
    context = None
    try:
        # Contexto con UA real + locale MX (evita variaciones de contenido)
        context = browser.new_context(
        ...
        # 1) Cargar el HOME (siempre resuelve; la ruta directa de inversión rebota)
        page.goto(CREDICLUB_HOME, timeout=45000, wait_until="domcontentloaded")
        page.wait_for_timeout(2500)
 
        # 2) Click en el enlace de inversión ("Conocer más") -> router de Framer
        def _en_pagina_inversion():
            try:
                return bool(re.search(r'Plazo de 364', page.locator("body").inner_text())) \
                    or "/inversi" in page.url
            except Exception:
                return False
 
        clicked = False
        try:
            link = page.locator('a[href*="inversi"]').first
            link.wait_for(state="attached", timeout=10000)
            link.click(timeout=8000)
            clicked = True
        except Exception as e:
            print("Crediclub: click con locator falló, intento por JS:", e)
 
        if not clicked or not _en_pagina_inversion():
            # Fallback: click directo por JS sobre el primer <a> con 'inversi'
            try:
                page.evaluate("""() => {
                    const a = [...document.querySelectorAll('a')]
                        .find(x => (x.getAttribute('href') || '').includes('inversi'));
                    if (a) a.click();
                }""")
            except Exception as e:
                print("Crediclub: JS click falló:", e)
 
        # 3) Esperar a que aparezcan las tasas (texto 'Plazo de 364' o 'A la Vista')
        try:
            page.wait_for_function(
                "() => { const t = document.body.innerText; "
                "return t.includes('Plazo de 364') || /A la Vista/i.test(t); }",
                timeout=15000
            )
        except Exception:
            print("Crediclub: timeout esperando las tasas; leyendo lo que haya...")
        ...
        contenido = page.locator("body").inner_text()
        print("Crediclub Playwright texto:", len(contenido), "chars | url:", page.url)
        tasas = _parsear_texto_supertasas(contenido)
        print("Crediclub/Supertasas detectadas:", tasas)
        return tasas
    except Exception as e:
        print("Error Crediclub/Supertasas Playwright:", e)
    finally:
        if context:
            try:
                context.close()
